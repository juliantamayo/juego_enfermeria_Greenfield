...
const Scene5Screen = () => {
  ...
  useScreenTitle("case1.scene5.screenTitle");
  ...
  const handleAnswerPress = (questionId: string, answerId: string, isCorrect?: boolean) => {
    ...
    feedbackTimeoutRef.current = setTimeout(() => {
      ...
      setMessage({
        title: t(
          finalHasPerfectScore ? "case1.scene5.messages.completed.title" : "case1.scene5.messages.review.title"
        ),
        text: t(
          finalHasPerfectScore ? "case1.scene5.messages.completed.text" : "case1.scene5.messages.review.text",
          {
  ...
  const handleCorrect = () => {
    ...
    setMessage({
      title: t("case1.scene5.messages.readyToCorrect.title"),
      text: t("case1.scene5.messages.readyToCorrect.text"),
  ...
  const handleSave = () => {
    if (!hasPerfectScore) {
      setMessage({
        title: t("case1.scene5.messages.review.title"),
        text: t("case1.scene5.messages.review.saveBlocked"),
    ...
    <ImageBackground source={require("@images/layout/background.png")} style={GlobalStyles.container} resizeMode="contain">
      ...
      >
        <ProgressBar
          ...
          max={totalQuestions}
          label={t("case1.scene5.questionCounter", { current: currentQuestionIndex + 1, total: totalQuestions })}
        />
          ...
          <>
            <View style={styles.resultCard}>
              <View style={styles.resultItem}>
                <Text style={styles.resultLabel}>{t("case1.scene5.results.correct")}</Text>
                <Text style={styles.resultValue}>{result.correct}</Text>
              </View>
              <View style={styles.resultItem}>
                <Text style={styles.resultLabel}>{t("case1.scene5.results.incorrect")}</Text>
        ...
        <View style={[styles.footer, { paddingBottom: Spacing.base + insets.bottom }]}>
          {shouldShowCorrectAction && (
            <View style={styles.footerActions}>
              <Button
                text={t("case1.scene5.actions.correct")}
              ...
              />
              <Button
                text={t("case1.scene5.actions.backToMenu")}
          ...
          {shouldShowSaveAction && (
            <Button
              text={t("case1.scene5.actions.save")}
              type={ButtonType.PRIMARY}
              style={styles.button}
              onPress={handleSave}
            />
